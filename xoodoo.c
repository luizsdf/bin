...
#define SWAP32(s, u, v) t = (s)[u], (s)[u] = (s)[v], (s)[v] = t
...
void absorb(uint32_t state[12], const uint8_t *message, size_t size)
{
    size_t i;
...
void squeeze(uint32_t state[12], uint8_t *digest, size_t size)
{
    size_t i;
    uint8_t *st = (uint8_t *) state;
    while (size >= 16) {
